...
public class AESEncryption extends DefaultEncryption
{
	private static final String CLASSNAME = "AESEncryption";
	private static ConsoleLogger logger =new ConsoleLogger();

	public String crypt(String enteredPassword)
	{
		return AESEncrypt.encrypt(enteredPassword);
	}

	/**
	 * arg1 encryptedPassword arg2 enteredPassword returns true if the encryptedPassword and 
	 * the enteredPassword matches false if the encryptedPassword and the enteredPassword does not match
	 */
	...
	public String decrypt(String encryptedPassword)
	{
		...
		try
		{
			decryptPassword = AESEncrypt.decrypt(encryptedPassword);
		}
		catch (Exception e)
		{
